...
function sample_posterior(rng::AbstractRNG, bsm::BSMModel{T, A, NamedTuple{(:x, :log_B, :b_ind, :bincounts, :μ, :P, :n), Vals}}, n_samples::Integer, n_burnin::Integer) where {T, A, Vals}
...
function sample_posterior(rng::AbstractRNG, bsm::BSMModel{T, A, NamedTuple{(:x, :log_B, :b_ind, :μ, :P, :n), Vals}}, n_samples::Integer, n_burnin::Integer) where {T, A, Vals}
    basis = BSplineKit.basis(bsm)
    K = length(basis)
